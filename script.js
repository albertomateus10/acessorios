...
const state = { search: '', Loja: [], Vendedor: [], Modelo: [], Mes: [], Nota: [], CodItem: [], OS: [] };
...
const pillInstances = {
    ...
    Modelo: new MultiPill({ label: 'Acessório', getter: () => state.Modelo, setter: v => state.Modelo = v }),
    Nota: new MultiPill({ label: 'Nota', getter: () => state.Nota, setter: v => state.Nota = v }),
    CodItem: new MultiPill({ label: 'Cód. Item', getter: () => state.CodItem, setter: v => state.CodItem = v }),
    OS: new MultiPill({ label: 'O.S.', getter: () => state.OS, setter: v => state.OS = v }),
};
(function mountPills() {
    const holder = document.getElementById('filters');
    ['Mes', 'Loja', 'Vendedor', 'Modelo', 'Nota', 'CodItem', 'OS'].forEach(k => {
...
function parseRows(headers, data) {
    const idx = {
        ...
        loja: findCol(headers, ['loja', 'Loja', 'LOJA', 'Empresa vendedora', 'EMPRESA VENDEDORA']),
        ...
        os: findCol(headers, ['O.S.', 'OS', 'Ordem de Serviço'])
    };
    ...
    pillInstances.OS.setOptions(allData.map(x => x.OS));
...
document.getElementById('clearAll').addEventListener('click', () => {
    withStableScroll(() => {
        ...
        state.Nota = []; state.CodItem = []; state.OS = [];
...
function applyFilters() {
    filtered = allData.filter(r => {
        ...
        const osOk = state.OS.length === 0 || state.OS.includes(r.OS);

        const q = state.search;
        const qOk = !q || [r.Descricao, r.OS, r.CodItem].some(vv => (vv || '').toString().toLowerCase().includes(q));

        return lojaOk && vendOk && modOk && mesOk && notaOk && codOk && osOk && qOk;
    });
    renderKpiGroups();
...
function renderInsights(rows) {
    ...
    const cards = [
        { icon: 'fa-tags', head: 'Acessório destaque', text: `${k.itemMaisVendido} lidera em quantidade.` },
...
function renderCharts(rows) {
    ...
    /* Agregação centralizada para Acessórios (Tooltip detalhado) */
    const statsByDesc = {};
    rows.forEach(r => {
        const k = r.Descricao || '—';
    ...
    charts.valorItem = new Chart(document.getElementById('valorItemChart').getContext('2d'), {
        ...
        options: { ...commonOpts, indexAxis: 'y', plugins: { ...commonOpts.plugins, legend: { display: false }, title: { display: true, text: 'Top 10 Acessórios (Valor R$)' }, datalabels: { ...datalabelsCenter(), formatter: v => BRL(v) }, tooltip: accessoryTooltip } }
    });
    ...
    charts.modelo = new Chart(document.getElementById('modeloChart').getContext('2d'), {
        ...
        options: { ...commonOpts, indexAxis: 'y', plugins: { ...commonOpts.plugins, legend: { display: false }, title: { display: true, text: 'Top 10 Acessórios (Quantidade)' }, datalabels: datalabelsCenter(), tooltip: accessoryTooltip } }
    });
